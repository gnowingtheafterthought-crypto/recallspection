# ================================================================
# 🧠 RECALLSPECTION API (Full Engine)
# ================================================================
import torch
...
app = FastAPI(title="Recallspection API")   # <-- THIS IS THE APP OBJECT
...
def validate_api_key(api_key: str = Security(api_key_header)):
    conn = sqlite3.connect('recallspection.db')
    c = conn.cursor()
    c.execute("SELECT id, plan FROM users WHERE api_key=? AND is_active=1", (api_key,))
    user = c.fetchone()
    conn.close()
    if not user:
