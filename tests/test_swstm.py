...
def test_flat_swtm_100_facts():
    ...
    # No explicit training needed if we have enough slots (2x facts)
    # but we can train a few epochs to improve separation (optional)
    # For this test, we just rely on the memory and assume it's good.
    # We'll measure accuracy.
...
# -----------------------------------------------------------------------------
# Test 2: Flat SWSTM with training (optional, but included)
# -----------------------------------------------------------------------------

def test_flat_swtm_training():
    """Test training loop improves accuracy."""
    keys, values = generate_facts(100)
    # Use a model directly, not the engine
    ...
    # Add facts to model (so that memory is populated)
...
def test_auto_mode_switch():
    ...
    # We can check the type of memory
